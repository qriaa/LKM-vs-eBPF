...
int main(int argc, char **argv) {
    ...
    skel = nf_firewall_bpf__open_and_load();
    if (!skel) {
        fprintf(stderr, "Failed to open and load BPF skeleton: %s\n", strerror(errno));
        return 1;
    }

    struct bpf_program *prog_to_attach = skel->progs.nf_firewall;
